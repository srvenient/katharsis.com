...
from app.api.shared.service import email_service
...
class AuthService:
    ...
    async def recover_password(self, email: str) -> JSONResponse:
        ...
        password_reset_token = security.generate_reset_password_token(email)
        ...
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content={"detail": "Password recovery instructions sent to your email"}
        )

    async def reset_password(self, body: NewPassword) -> JSONResponse:
        ...
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content={"detail": "Password reset successful"}
        )
